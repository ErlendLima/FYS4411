...
space must be explored, which is done through Monte Carlo simulation, more
...
the entire probability density each step. If the ratio \(\omega\) is greater
than a
uniformly distributed number \(\theta \in [0, 1]\), the move is accepted.

The disadvantage with this naive approach is that the proposed move is random.
The configuration space is explored more effectively by instead moving towards
regions of higher probability density. As explained in SOURCE, this is achieved
by adding a quantum force on the particle \textit{k}

\begin{align*}
  \vb{F}_{k} = 2\frac{1}{\Psi_{t}}\nabla_{k}\Psi_{t}
\end{align*}
